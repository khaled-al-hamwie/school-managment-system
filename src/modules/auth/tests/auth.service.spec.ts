...
describe("AuthService", () => {
    ...
    describe("sign toke", () => {
        it("should create with manager payload", async () => {
            const managerPayload: ManagerPayload = {
                credentail_id: 1,
                user_name: "test",
                manager_id: 1,
            };
            const output = await service.signToken(managerPayload);
            expect(output.access_token).not.toBeNull();
            const decoded = await jwt.verify(output.access_token, {
        ...
        it("should create with teacher payload", async () => {
            const teacherPayload: TeacherPayload = {
                credentail_id: 1,
                user_name: "test",
                teacher_id: 1,
            };
            const output = await service.signToken(teacherPayload);
            expect(output.access_token).not.toBeNull();
            const decoded = await jwt.verify(output.access_token, {
                secret: process.env.JWTKEY,
            });
        ...
        it("should create with student payload", async () => {
            ...
            const output = await service.signToken(studentPayload);
            expect(output.access_token).not.toBeNull();
            const decoded = await jwt.verify(output.access_token, {
                secret: process.env.JWTKEY,
            });
